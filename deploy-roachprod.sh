...
roachprod run ${NIFI} "curl -s https://archive.apache.org/dist/nifi/$NIFI_VERSION/nifi-$NIFI_VERSION-bin.tar.gz | sudo tar -C /opt/nifi -xz"
...
#nifi.zookeeper.connect.string=$CLUSTER-0001:2181,$CLUSTER-0002:2181,$CLUSTER-0003:2181
roachprod run ${NIFI} 'replaceString=true && sudo sed -i "s/\(nifi\.state\.management\.embedded\.zookeeper\.start=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString=true && sudo sed -i "s/\(nifi\.cluster\.is\.node=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString=11443 && sudo sed -i "s/\(nifi\.cluster\.node\.protocol\.port=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString=9090 && sudo sed -i "s/\(nifi\.web\.http\.port=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString=`hostname` && sudo sed -i "s/\(nifi\.web\.http\.host=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString=`hostname` && sudo sed -i "s/\(nifi\.cluster\.node\.address=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString="30 sec" && sudo sed -i "s/\(nifi\.cluster\.flow\.election\.max\.wait\.time=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'
roachprod run ${NIFI} 'replaceString='"$CLUSTER"'-0001:2181,'"$CLUSTER"'-0002:2181,'"$CLUSTER"'-0003:2181 && sudo sed -i "s/\(nifi\.zookeeper\.connect\.string=\).*\$/\1${replaceString}/" /opt/nifi/nifi-current/conf/nifi.properties'

#### zookeeper.properties
# Remove default entry, add servers to properties file

#head -n -2 /opt/nifi/nifi-current/conf/zookeeper.properties
roachprod run ${NIFI} 'sudo sed -i "/server\.1=/d" /opt/nifi/nifi-current/conf/zookeeper.properties'

roachprod run ${NIFI} 'sudo chmod 777 /opt/nifi/nifi-current/conf/zookeeper.properties'
roachprod run ${NIFI} 'sudo echo -e "server.1='"$CLUSTER"'-0001:2888:3888;2181" >> /opt/nifi/nifi-current/conf/zookeeper.properties'
roachprod run ${NIFI} 'sudo echo -e "server.2='"$CLUSTER"'-0002:2888:3888;2181" >> /opt/nifi/nifi-current/conf/zookeeper.properties'
roachprod run ${NIFI} 'sudo echo -e "server.3='"$CLUSTER"'-0003:2888:3888;2181" >> /opt/nifi/nifi-current/conf/zookeeper.properties'

# Create zookeeper state folder on all hosts
roachprod run ${NIFI} 'sudo mkdir -p /opt/nifi/nifi-current/state/zookeeper'
roachprod run ${NIFI} 'sudo chmod -R 777 /opt/nifi/nifi-current/state/zookeeper'

#### Add Zookeeper ids on all hosts
roachprod run ${CLUSTER}:1 'sudo echo 1 > /opt/nifi/nifi-current/state/zookeeper/myid'
roachprod run ${CLUSTER}:2 'sudo echo 2 > /opt/nifi/nifi-current/state/zookeeper/myid'
roachprod run ${CLUSTER}:3 'sudo echo 3 > /opt/nifi/nifi-current/state/zookeeper/myid'

#### State Management file
# Under cluster-provider -> provider id add the following
# <property name="Connect String"></property>
roachprod run ${NIFI} 'sudo sed -i "s/<property name=\"Connect String\"><\/property>/<property name=\"Connect String\">'"$CLUSTER"'-0001:2181,'"$CLUSTER"'-0002:2181,'"$CLUSTER"'-0003:2181<\/property>/" /opt/nifi/nifi-current/conf/state-management.xml'

roachprod run ${CLUSTER} 'sudo rm -f /opt/nifi/nifi-current/conf/flow.xml.gz'

#roachprod get ${CLUSTER} /opt/nifi/nifi-current/conf/nifi.properties ./nifi.properties
...
echo -e ""
echo -e "NiFi Status:"
roachprod run ${NIFI} 'sudo /opt/nifi/nifi-current/bin/nifi.sh status'
echo -e ""
